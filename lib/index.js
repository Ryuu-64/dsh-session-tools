...
/**
* Name a session in tool output: its title when it has one, always with the id
* the person needs to find it in the sidebar. The `@[label](dsh-session:…)`
* mention syntax is NOT used here — it is parsed only from the person's own
* composer input, so inside tool output it would show as raw markdown.
