...
/**
* Wait until a message this plugin queued has been handed to the driver. A
* session can still report `idle` the instant the message is queued, and a wait
* that accepts that would return the answer from the PREVIOUS turn — so the
* condition is "this message left the inbox AND the session is idle again".
*
* The inbox is public Agent surface: `inbox.nextTurn` is a readonly list of the
* messages still waiting, and each carries its own id.
* @param agent - the live target agent.
* @param messageId - the id of the message we just queued.
* @param budgetMs - how long to keep polling before giving up.
* @returns true when the message was taken, false on timeout.
*/
async function waitUntilConsumed(agent, messageId, budgetMs) {
	const deadline = Date.now() + budgetMs;
	const stillQueued = () => {
		const pending = agent.inbox?.nextTurn;
		// An unrecognisable inbox shape cannot prove anything; report a timeout
		// rather than a completion we did not observe.
		if (!Array.isArray(pending)) return undefined;
		return pending.some((queued) => queued?.id === messageId);
	};
	for (;;) {
		const queued = stillQueued();
		if (queued === false && agent.status !== "running") return true;
...
async function resolveTargetAgent(ctx, sessionId, callerAgent) {
	...
	if (record.header.cwd === void 0) throw new Error(`session_send cannot open ${JSON.stringify(sessionId)}: this saved session does not record a working directory, and reopening it needs one. Send to a session that has a directory, or ask the person to open that session once.`);
...
function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			// Waiting for our own message, not merely for quiet. The agent can still
			// look idle the instant the message is queued, so an idle wait alone
			// would return the PREVIOUS answer; wait until this message has left the
			// inbox first. Both signals are public Agent surface.
			const startedAt = Date.now();
			const timeoutMs = waitBudgetMs(args.timeoutMs);
			const consumed = await waitUntilConsumed(target, sent.id, timeoutMs);
